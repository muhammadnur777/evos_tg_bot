...
class Database:
    ...
    def add_pr_description(self, product_name, description):
    ...
    def get_order(self, tg_id: int):
        sql = '''SELECT user_id, product_id, quantity, product_type FROM Orders WHERE user_id = (SELECT id FROM Users WHERE telegram_id = ?)'''
        result = self.execute(sql=sql, parameters=(tg_id,), fetchall=True)
        print(f"get_order result: {result}")  # Отладочный вывод
        return result
